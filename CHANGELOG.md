...
### Changed
- Updated copyright year from 2024 to 2025 in LICENSE.
- Improved demo and skeleton structure (no changes to core package functionality).

## [5.0.1] - 2024/5/14

### Changed
- Fixed installer program. Added process to remove `prototypes/`,`__tests__/`,`phpunit-printer.yml`,`phpunit.xml` after installation.
- Added `client/package-lock.json` to skeleton.
...
- Renamed methods to more appropriate names.
  |before|after|
  |--|--|
...
### Changed
- Renamed the file (directory) path validation function name from "directory_path" to "is_path".  

## [4.1.7] - 2023/8/29

### Changed
- Fixed directory creation method to return true if the directory creation succeeds and false if it fails.  
...
- Refactored Util\RestClient member variable names.
  |Before|After|
  |--|--|
...
- Added a method to get the number of GIF frames in the "\X\Util\ImageHelper" class.
...
### Changed
- Deleted PID from log output from "\X\Util\Logger".
- Deleted printWithoutPath method from "\X\Util\Logger".
...
### Added
- Added form_validation_test action to the sample test controller.
...
### Changed
- Fixed methods for omitting long strings (\X\UtilStringHelper#ellipsis) to support Unicode.
...
### Changed
- Added ability to specify whether lock is enabled or disabled when deleting a file.
...
### Changed
- Changed query logging behavior.
  ```php
  use \X\Util\Logger;
...
### Changed
- Changed random character generation function name to camel case.
...
### Fixed
- Fixed the issue text not recognized after '&' in case of PUT request.

## [3.9.5] - 2021/4/8

### Changed
- Refactored the skeleton.
...
### Changed
- Changed image resizing features(\X\Util\ImageHelper).
...
### Changed
- Removed unused paginate method from Model class.
...
### Changed
- Fixed project creation process.

## [3.6.7] - 2020/11/16

### Changed
- Prepended a slash to the PID of the log(\X\Util\Logger).  
...
### Changed
- Added PID to log message(\X\Util\Logger).
...
### Changed
- Removed class and function names from the log(\X\Util\Logger).
...
### Added
- Added a time stamp to the log message output to the CLI(\X\Util\Logger#printWithoutPath).
...
### Added
- Added encryption key to the parameter of hash conversion method.
